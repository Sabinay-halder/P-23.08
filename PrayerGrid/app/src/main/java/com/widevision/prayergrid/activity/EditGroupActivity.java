...
import com.widevision.prayergrid.dao.EditGroupDao;
import com.widevision.prayergrid.dao.GsonClass;
import com.widevision.prayergrid.dao.ViewGroupProfileDao;
import com.widevision.prayergrid.fragment.GroupProfileFragment;
...
public class EditGroupActivity extends HideKeyActivity {
    ...
    @Bind(R.id.type_ministry)
    RadioButton type_ministry;
    @Bind(R.id.type_people)
    RadioButton type_people;
    @Bind(R.id.type_workplace)
    RadioButton type_workplace;

    @Bind(R.id.invite)
    RadioButton inviteRadio;
    @Bind(R.id.public_radio)
    RadioButton publicRadio;

    @Bind(R.id.give_tab_yes)
    RadioButton giveYes;
    @Bind(R.id.give_tab_no)
    RadioButton giveNo;

    @Bind(R.id.contact_no)
    RadioButton contactNo;
    @Bind(R.id.contact_yes)
    RadioButton contactYes;


    @Bind(R.id.meeting_yes)
    RadioButton meetingYes;
    @Bind(R.id.meeting_no)
    RadioButton meetingNo;

    @Bind(R.id.location_yes)
    RadioButton locationYes;
    @Bind(R.id.location_no)
    RadioButton locationNo;
    ...
    @Bind(R.id.volunteer_no)
    RadioButton volunteerNo;
    @Bind(R.id.volunteer_yes)
    RadioButton volunteerYes;

    @Bind(R.id.member_no)
    RadioButton memberNo;
    @Bind(R.id.member_yes)
    RadioButton memberYes;

    /*The type field is required.
    * The privacy field is required.*/
    private GsonClass.Post_info item = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gridAdapter = new GridAdapter(EditGroupActivity.this);
        gridAdapterProfile = new GridAdapterProfile(EditGroupActivity.this);
        ...
        mCountrySp.setAdapter(new ArrayAdapter<String>(EditGroupActivity.this, R.layout.row_spinner, HomeActivity.countries) {
            @Override
    ...
    private void setAllField() {
        String groupType = item.type;
        if (groupType.equals("1")) {
            type_ministry.setChecked(true);
            type_ministry.setSelected(true);
        } else if (groupType.equals("2")) {
            type_workplace.setChecked(true);
            type_workplace.setSelected(true);
        } else if (groupType.equals("3")) {
            type_people.setChecked(true);
            type_people.setSelected(true);
        }
        String groupName = item.name;
        mGroupNameEdt.setText(groupName);

        String privacyStr = item.privacy;
        if (privacyStr.equals("0")) {
            inviteRadio.setSelected(true);
            inviteRadio.setChecked(true);
        } else {
            publicRadio.setChecked(true);
            publicRadio.setSelected(true);
        }
        String giveTab = item.settings.give;
        if (giveTab.equals("0")) {
            giveNo.setSelected(true);
            giveNo.setChecked(true);
        } else {
            giveYes.setSelected(true);
            giveYes.setChecked(true);
        }

        String giveLink = item.details.give.link;
        mGiveLinkEdt.setText(giveLink);

        String giveContact = item.settings.contact;
        if (giveContact.equals("0")) {
            contactNo.setChecked(true);
            contactNo.setSelected(true);
        } else {
            contactYes.setChecked(true);
            contactYes.setSelected(true);
        }
        String email = item.details.contact_email;
        mEmailEdt.setText(email);

        String time = item.settings.time;
        if (time.equals("0")) {
            meetingNo.setSelected(true);
            meetingNo.setChecked(true);
        } else {
            meetingYes.setSelected(true);
            meetingYes.setChecked(true);
        }
        String meetingTime = item.details.time;
        mMeetingTimeEdt.setText(meetingTime);
        String news = item.news;
        mNewsEdt.setText(news);

        String location = item.settings.location;
        if (location.equals("0")) {
            locationNo.setSelected(true);
            locationNo.setChecked(true);
        } else {
            locationYes.setSelected(true);
            locationYes.setChecked(true);
        }

        String address = item.details.location.address;
        String city = item.details.location.city;
        String state = item.details.location.state;

        mAddressEdt.setText(address);
        mCityEdt.setText(city);
        mStateEdt.setText(state);
//country
        String country = item.details.location.country;
        ...
        String number = item.details.phone;
        mNumberEdt.setText(number);
        String volunteer = item.details.volunteer.link;
        mVolunteerLinkEdt.setText(volunteer);

        String call = item.settings.phone;
        if (call.equals("0")) {
            callNo.setChecked(true);
            callNo.setSelected(true);
        } else {
            callYes.setChecked(true);
            callYes.setSelected(true);
        }

        String vol = item.settings.volunteer;
        if (vol.equals("0")) {
            volunteerNo.setChecked(true);
            volunteerNo.setSelected(true);
        } else {
            volunteerYes.setChecked(true);
            volunteerYes.setSelected(true);
        }
        String member = item.settings.memberapprove;
        if (member.equals("0")) {
            memberNo.setSelected(true);
            memberNo.setChecked(true);
        } else {
            memberYes.setSelected(true);
            memberYes.setChecked(true);
        }
        String volunteerImage = item.details.volunteer.image;
        if (volunteerImage.equals("volunteer1")) {
            v1.setVisibility(View.VISIBLE);
            v2.setVisibility(View.GONE);
            v3.setVisibility(View.GONE);
        } else if (volunteerImage.equals("volunteer2")) {
            v1.setVisibility(View.GONE);
            v2.setVisibility(View.VISIBLE);
            v3.setVisibility(View.GONE);
        } else {
    ...
    private void editProfile() {
        if (extension.executeStrategy(EditGroupActivity.this, "", ValidationTemplate.INTERNET)) {
            ViewGroupProfileDao dao = new ViewGroupProfileDao(Constant._id, group_id);
            dao.query(new AsyncCallback<GsonClass>() {
                @Override
                public void onOperationCompleted(GsonClass result, Exception e) {
                    if (e == null && result != null) {
                        if (result.success.equals("1")) {
                            item = result.group_info;
                            setAllField();
                        } else {
    ...
    private void checkValidation() {
        ...
        if (typeCheck < 0) {
            Constant.setAlert(EditGroupActivity.this, "The type field is required.");
            return;
        }
        int p = mPrivacyRadionGroup.getCheckedRadioButtonId();
        if (p < 0) {
            Constant.setAlert(EditGroupActivity.this, "The privacy field is required.");
            return;
        }
        ...
        if (groupName.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please include name for your group.");
            return;
        } else if (giveLink.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter give link.");
            return;
        } else if (contactEmail.isEmpty() || !extension.executeStrategy(EditGroupActivity.this, contactEmail, ValidationTemplate.EMAIL)) {
            if (contactEmail.isEmpty()) {
                Constant.setAlert(EditGroupActivity.this, "Please enter your email address");
            } else {
                Constant.setAlert(EditGroupActivity.this, "Please enter valid email address");
            }
            return;
        } else if (meetingTime.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter your meeting time.");
            return;
        } else if (news.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter News field.");
            return;
        } else if (country.equals("All Countries")) {
            Constant.setAlert(EditGroupActivity.this, "Please select your country.");
            return;
        } else if (address.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter your address.");
            return;
        } else if (city.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter your city.");
            return;
        } else if (state.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter your state.");
            return;
        } else if (phone.isEmpty() || !extension.executeStrategy(EditGroupActivity.this, phone, ValidationTemplate.isnumber)) {
            if (phone.isEmpty()) {
                Constant.setAlert(EditGroupActivity.this, "Please enter your phone number.");
            } else {
                Constant.setAlert(EditGroupActivity.this, "Please enter valid phone number.");
            }
            return;
        } else if (volunteer.isEmpty()) {
            Constant.setAlert(EditGroupActivity.this, "Please enter volunteer link.");
            return;
        }
...
*/
        ...
        if (mCoverPhotoPath.equals("")) {
            if (selected_cover_pos == -1) {
                Constant.setAlert(EditGroupActivity.this, "Please upload Cover image.");
                return;
            } else {
        ...
        if (mProfilePhotoPath.equals("")) {
            if (selected_profile_pos == -1) {
                Constant.setAlert(EditGroupActivity.this, "Please upload Profile image.");
                return;
            } else {
        ...
        if (extension.executeStrategy(EditGroupActivity.this, "", ValidationTemplate.INTERNET)) {
            progressLoaderHelper.showProgress(EditGroupActivity.this);
            EditGroupDao dao = null;
            if (!mCoverPhotoPath.equals("") && !mProfilePhotoPath.equals("")) {
                dao = new EditGroupDao(Constant._id, group_id, groupName, "" + gType, "" + privacreType, "" + detailJonsObject.toString(), "" + settingJsonObject.toString(), news, "status", new File(mCoverPhotoPath), new File(mProfilePhotoPath));
            } else if (cover_byte_array != null && profile_byte_array != null) {
                dao = new EditGroupDao(Constant._id, group_id, groupName, "" + gType, "" + privacreType, "" + detailJonsObject.toString(), "" + settingJsonObject.toString(), news, "status", cover_byte_array, profile_byte_array);
            } else if (mProfilePhotoPath.equals("") && profile_byte_array != null) {
                dao = new EditGroupDao(Constant._id, group_id, groupName, "" + gType, "" + privacreType, "" + detailJonsObject.toString(), "" + settingJsonObject.toString(), news, "status", new File(mCoverPhotoPath), profile_byte_array);
            } else if (mCoverPhotoPath.equals("") && cover_byte_array != null) {
                dao = new EditGroupDao(Constant._id, group_id, groupName, "" + gType, "" + privacreType, "" + detailJonsObject.toString(), "" + settingJsonObject.toString(), news, "status", cover_byte_array, new File(mProfilePhotoPath));
            } else {
                Constant.setAlert(EditGroupActivity.this, "Select cover & profile pic.");
                return;
            }
            ...
            dao.query(new AsyncCallback<GsonClass>() {
                ...
                public void onOperationCompleted(GsonClass result, Exception e) {
                    ...
                    if (e == null && result != null) {
                        if (result.success.equals("1")) {
                            finish();
                            Toast.makeText(EditGroupActivity.this, "Group Created successfully.", Toast.LENGTH_SHORT).show();
                        } else {
                            Constant.setAlert(EditGroupActivity.this, result.message);
                        }
                    } else {
                        Constant.setAlert(EditGroupActivity.this, getString(R.string.wrong));
                    }
                }
            });
        } else {
            Constant.setAlert(EditGroupActivity.this, getString(R.string.no_internet));
        }
    }

    private void init() {
        ButterKnife.bind(EditGroupActivity.this);
    ...
    private AlertDialog alert() {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(EditGroupActivity.this, android.R.layout.select_dialog_item, items);
