...
import com.widevision.prayergrid.dao.GetGroupMemberDao;
...
public class GroupMemberListActivity extends HideKeyActivity {
    ...
    private void getMemberList() {
        if (extension.executeStrategy(GroupMemberListActivity.this, "", Implementation.INTERNET)) {
            progressLoaderHelper.showProgress(GroupMemberListActivity.this);
            GetGroupMemberDao memberDao = new GetGroupMemberDao(Constant._id, g_id);
                    ...
                    if (e == null && result != null) {
                        if (result.success.equals("1")) {
                            if (result.post_info != null && result.post_info.size() != 0) {
                                CustomListAdapter adapter = new CustomListAdapter(GroupMemberListActivity.this, result.post_info);
                                mListView.setAdapter(adapter);
                            } else {
                                finish();
                                Toast.makeText(GroupMemberListActivity.this, "No member in this Group.", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            finish();
                            Toast.makeText(GroupMemberListActivity.this, "No member in this Group.", Toast.LENGTH_SHORT).show();
                        ...
                        Constant.setAlert(GroupMemberListActivity.this, getResources().getString(R.string.wrong));
                    }
                }
            });
        } else {
            Constant.setAlert(GroupMemberListActivity.this, getString(R.string.no_internet));
        }
    }
